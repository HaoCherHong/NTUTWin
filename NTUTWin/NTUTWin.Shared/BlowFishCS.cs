...
namespace BlowFishCS
{
    class BlowFish
    {
        ...
        public string Decrypt_CBC(string ct)
        {
            ...
            var bytes = Decrypt_CBC(HexToByte(ct.Substring(16)));
            return Encoding.UTF8.GetString(bytes, 0, bytes.Length).Replace("\0", "");
        }
        ...
        {
            return Convert.ToBase64String(Encrypt_ECB(Encoding.UTF8.GetBytes(pt)));
        ...
        {
            var bytes = Decrypt_ECB(HexToByte(ct));
            return Encoding.UTF8.GetString(bytes, 0, bytes.Length).Replace("\0", "");
        }
        ...
        public string Encrypt_CTR(string pt)
        {
            if (!IVSet)
                SetRandomIV();
            return ByteToHex(InitVector) + ByteToHex(Crypt_CTR(Encoding.UTF8.GetBytes(pt), 2));
        }
        ...
        {
            IV = HexToByte(ct.Substring(0, 16));
            var bytes = Crypt_CTR(HexToByte(ct.Substring(16)), 2);
            return Encoding.UTF8.GetString(bytes, 0, bytes.Length).Replace("\0", "");
        }
